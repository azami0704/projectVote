...
if(isset($_SESSION['user'])){
  $userLog = find('projectvote_log',['daily_subject_id'=>$_GET['id'],'user_id'=>$_SESSION['user']['id']]);
}
      ...
      <ul class="result-option-list">
        ...
        if(!empty($optionId)){
          ...
          foreach($optionId as $value){
            $vote=find('projectvote_subject_daily_options',['id'=>$value])['opt'];
            $votedOpt[]="'{$vote}'";
...
<script>
    ...
    //撈圖表用資料
    axios.get("./api/result_api.php?daily&id=<?=$_GET['id']?>")
    .then(res=>{
      // console.log(res);
      renderC3(res.data)
    })
