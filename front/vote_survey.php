...
<div class="container-xxl pb-5 mt-3">
    ...
    <form action="./api/vote_survey.php" method="post" class="vote-survey-form w-50 mx-auto position-relative active" id="vote-survey-form">
   ...
   $printInfo=false;
    if(isset($_SESSION['user'])){
        //私人投票進行權限判斷
        if($survey['private']==2){
            $authCheck = find('projectvote_subject_users',['subject_id'=>$_GET['id'],'account'=>$_SESSION['user']['account']]);
            //確認有權限顯示投票鈕
            if(empty($authCheck)){
                $printInfo = true;
                $infoTitle="無投票權限!";
                $btnLeft = "<a href='?do=mysurvey' class='btn btn-main btn-lg fw-bold'>我的清單</a>";
                $btnRight = "";
            }
        }else{
            //判斷投票是否啟用中
        ...
        //判斷是否投過票
        $votedLogs = all('projectvote_log',['user_id'=>$_SESSION['user']['id'],'subject_id'=>$_GET['id']]);
        if(!empty($votedLogs)){
            $printInfo = true;
            $infoTitle="已經投過票囉!";
            $btnLeft = "<a href='?do=main' class='btn btn-secondary btn-lg fw-bold'>看看別的</a>";
