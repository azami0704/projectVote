...
//更新subject票數
$subject = find('projectvote_subject_daily',$_POST['id']);
$subjectVote = $subject['vote']+1;
echo "subject票數:$subjectVote";
update('projectvote_subject_daily',['vote'=>$subjectVote],$_POST['id']);

//更新option票數
foreach($_POST['opt'] as $optId){
    $options = find('projectvote_subject_daily_options',$optId);
    $optionNum=$options['vote']+1;
    // echo "{$options['opt']}選單票數:$optionNum";
    update('projectvote_subject_daily_options',['vote'=>$optionNum],$optId);
}
...
// 'option_id'=>$opts
// ]);
header("location:../index.php?do=survey_result_daily&id={$_POST['id']}");
